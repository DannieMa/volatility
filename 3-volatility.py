...
# def getVola_year(df_ret,period,n):
#     """
#     计算年频波动
#     :param df_ret:
#     :param period:
...
#     vola=df_ret.rolling(period).std()*np.sqrt(n)
#     vola=vola.reset_index()
#     vola['year']=vola.date.apply(lambda x:str(x)[:4])
#     vola=vola.groupby('year').last().set_index('date')
#     vola=vola.dropna(how='all')
#
#     return vola
...
def save_vola_yearly():
    ...
    ret_q=df_1d/df_1d.shift(60)-1   #季度收益
    vola_q_df=getVola_month(ret_q,20,4)
    ...
    for y in range(1,9,1):
        ret_y=df_1d/df_1d.shift(240*y)-1
        vola_y_df=getVola_month(ret_y,20,1/y)
        arrays = [['vola_%dy'%y] * len(vola_y_df.columns), vola_y_df.columns.to_list()]
        vola_y_df.columns = pd.MultiIndex.from_arrays(arrays, names=('vola_proxy', 'code'))
        para_df.append(vola_y_df)
    return pd.concat(para_df,axis=1)


if __name__=='__main__':

    # daily_path='volatility/vola_daily.xlsx'
    # monthly_path='volatility/vola_monthly.xlsx'
    # yearly_path='volatility/vola_yearly.xlsx'
    ...
    proxy_ls_map={'vola_10min':'10分钟', 'vola_1h':'1小时', 'vola_1min':'1分钟', 'vola_30min':'30分钟', 'vola_5min':'5分钟',
                  ...
                  'vola_1y':'年频', 'vola_2y':'2年', 'vola_3y':'3年', 'vola_4y':'4年', 'vola_5y':'5年', 'vola_6y':'6年',
                 'vola_7y':'7年', 'vola_8y':'8年', 'vola_q':'季频'}
    proxy_ls=['1分钟','5分钟','10分钟','30分钟','1小时','日频','周频','双周','月频','季频','年频','2年','3年','4年','5年','6年','7年','8年']

    vola_daily = save_vola_daily()
    vola_monthly = save_vola_monthly()
    vola_yearly = save_vola_yearly()

    data_path='volatility1/volatility.xlsx'
    writer = pd.ExcelWriter(data_path)
    for code in code_ls_map.keys():
        para=[]
        para.append(vola_daily.loc[:,(slice(None),code)].droplevel('code', axis=1))
        para.append(vola_monthly.loc[:,(slice(None),code)].droplevel('code', axis=1))
        para.append(vola_yearly.loc[:,(slice(None),code)].droplevel('code', axis=1))
        para_df=pd.concat(para,axis=1)
        para_df=para_df.loc[para_df.index>=20100101,]
        para_df.columns=[proxy_ls_map[x] for x in para_df.columns]
        para_df=para_df[proxy_ls]
        para_df.to_excel(writer, code_ls_map[code])
        para_df.index=[pd.to_datetime(str(x))for x in para_df.index]
        para_df.plot(title='指数不同收益频率下的年化波动率-%s'%code_ls_map[code],figsize=(10,5))
        plt.savefig('volatility1/vola_plot_%s.jpg'%code_ls_map[code])
        plt.close('all')

    writer.save()


    # vola_daily_df=vola_daily.stack().reset_index()
    # writer_daily = pd.ExcelWriter(daily_path)
    # for code,data in vola_daily_df.groupby('code'):
    #     data.to_excel(writer_daily, code)
    # writer_daily.save()
    #
    #
    # vola_monthly_df=vola_monthly.stack().reset_index()
    # writer_monthly = pd.ExcelWriter(monthly_path)
    # for code,data in vola_monthly_df.groupby('code'):
    #     data.to_excel(writer_monthly, code)
    # writer_monthly .save()


    # vola_yearly_df = vola_yearly.stack().reset_index()
    # writer_yearly = pd.ExcelWriter(yearly_path)
    # for code, data in vola_yearly_df.groupby('code'):
    #     data.to_excel(writer_yearly, code)
    # writer_yearly.save()
